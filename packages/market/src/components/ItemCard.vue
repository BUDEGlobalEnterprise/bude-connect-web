...
const conditionConfig = computed(() => {
  const config: Record<string, { variant: "success" | "info" | "warning" | "default"; icon: string }> = {
    New: { variant: "success", icon: "✨" },
    "Like New": { variant: "info", icon: "👌" },
    "Good": { variant: "info", icon: "👍" },
    "Fair": { variant: "warning", icon: "📦" },
    "Open Box": { variant: "info", icon: "📭" },
...
const listingTypeConfig = computed(() => {
  ...
  return config[props.item.listing_type] || config["Sell"];
});

// Format price with compact notation for large values
const displayPrice = computed(() => {
  const price = props.item.standard_rate || props.item.price || 0;
...
<template>
  <RouterLink
    :to="`/listing/${item.name || item.item_code}`"
  ...
  >
    ...
    <div class="relative aspect-square overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div v-if="item.distance_km" class="absolute top-3 right-3">
    ...
    <div class="p-4">
      ...
      <div class="flex items-end justify-between gap-2">
        <div>
          ...
          <span v-if="item.listing_type === 'Rent'" class="text-xs text-gray-500 ml-1">/day</span>
        </div>
