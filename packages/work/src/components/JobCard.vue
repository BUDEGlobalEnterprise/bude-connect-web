...
const postedTime = computed(() => {
  return timeAgo(props.job.created_at || props.job.created);
});
  ...
  >
    ...
    <div class="flex items-start justify-between gap-4 mb-4">
      ...
      <div class="text-right shrink-0">
        ...
        <p class="text-lg font-bold text-gradient">{{ job.budget_range || "Flexible" }}</p>
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      <span
        v-for="(skill, index) in (job.skills_required || []).slice(0, 4)"
        :key="skill"
      ...
      <span
        v-if="(job.skills_required || []).length > 4"
